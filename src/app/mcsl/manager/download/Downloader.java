...
import app.mcsl.event.DownloadFinishEvent;
...
public class Downloader {
    ...
    public Downloader(String fileUrl, String destinationDir, int threadCount) {
        ...
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (finishEvent != null) finishEvent.onFinished(status);
            }
        } catch (IOException e) {
            Logger.exception(e);
        }
    ...
    private void checkRunning() {
        ...
        if (status != DownloadStatus.CANCELLED && status != DownloadStatus.ERROR) status = DownloadStatus.SUCCESS;
        timer.cancel();
